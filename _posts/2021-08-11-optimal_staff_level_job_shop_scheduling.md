...
![image](dp job-shop scheduling.png){width="15cm" height="7cm"}
...
at the start of each shift is given by $$S_t := (V_t, R_t)$$
...
$$X^t := \{x^t_j: \text{increase/decrease in number of staff for service type } j \text{ in shift } t,\,\, \forall j=1,\ldots,J\}$$
...
[2.3](#sse:immed-cost){reference-type="ref" reference="sse:immed-cost"}
and is realized before the start of next shift, the second component is
...
to Section [2.5](#sse:evolution){reference-type="ref"
reference="sse:evolution"}.\
...
representing the system's three sources of randomness (see Section
[1](#se:rough-solution){reference-type="ref"
reference="se:rough-solution"}). Recall that $S_t$ summarized vehicle
...
expected time of finishing service type $j$. NOTE that assuming an
...
[3](#se:sub-job_shop){reference-type="ref" reference="se:sub-job_shop"}.
...
optimization community. The reference section
[\[se:refer\]](#se:refer){reference-type="ref" reference="se:refer"} at
...
-   $s_{i,j,k}$ start time of service type $j$ of vehicle $i$ for staff
    $k$ of type $j$

-   $c_{i,j,k}$ completion time service type $j$ of vehicle $i$ for
    staff $k$ of type $j$

-   $y_{i, i', j, k}$ is 1 if service type $j$ of vehicle $i$ precedes
    service type $j$ of vehicle $i'$ for staff $k$ of type $j$

-   $c_i$ completion time of serving vehicle $i$
